...
- [ ] 2. Phase 1 — Build the latent behind the Identity subsystem (dormant/shadow mode)
  ...
  - [ ] 2.11 Checkpoint — verify Phase 1
...
- [ ] 3. Phase 2 — Latent-primary render path (flagged, A/B)
  ...
  - [ ] 3.4 Make the contract fatal on the latent path
    ...
    - **PARTIAL.** `render_from_latent` already asserts `mode='fatal'` at the B→D boundary (3.1). Remaining: confirm/centralize fatal-on-latent-path policy doc. Low risk; the assertion is live.
    - _Requirements: 3.1, 3.3, 9.5_

  - [ ] 3.5 Wire A/B comparison via render_mode_override
    - In `face_os/ab_validation.py`, wire latent-vs-legacy comparison through `process_frame(..., render_mode_override=...)`/`compare_render_methods` for SSIM, LAB drift, sharpness, and flicker, so promotion can be gated on no regression.
    - **DEFERRED to Phase 2B.** A real-video A/B was run (latent vs legacy face-bbox mean 0.93×, SSIM(latent,legacy)=0.9947 — latent ~indistinguishable from legacy, see corrected Phase 2A note). Formal ABComparator wiring is most useful AFTER the 2B mesh-normal fix, so it measures a real candidate rather than a render that collapses onto legacy.
    - _Requirements: 7.4_
  ...
  - [ ]* 3.10 Write property test for frame contract on synthesized output
  ...
  - [ ]* 3.11 Add TestLatentDrivesRender and TestSubsystemBoundaries (fast subset)
  ...
  - [ ] 3.12 Checkpoint — verify Phase 2
