...
Modes: local (default), remote (colab), sync-only, auto-pilot, tunnel."""
...
def main():
    ...
    actions = [args.memory_report, args.gpu_info, args.setup_colab,
    ...
    if args.memory_report:
        ...
        for k, v in memory_report().items(): print(f"  {k}: {v}"); return

    if args.gpu_info:
        from .colab import gpu_info
        for k, v in gpu_info().items(): print(f"  {k}: {v}"); return

    if args.setup_colab:
        from .colab import setup, start_tunnel
        s = setup()
        u = start_tunnel()
