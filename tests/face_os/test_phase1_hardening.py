...
class TestSystemIdentifiability:
    ...
    def test_identity_state_type_distinguishes_faces(self):
        """IdentityStateType from IdentityEstimator should distinguish faces."""
        estimator_a = IdentityEstimator()
        estimator_b = IdentityEstimator()

        face_a = _make_face((100, 100, 100))
        face_b = _make_face((200, 180, 160))

        estimator_a.set_anchor(face_a)
        estimator_b.set_anchor(face_b)

        geo_a = _make_geometry_state(face_a)
        geo_b = _make_geometry_state(face_b)

        # Run estimator multiple times
        for _ in range(20):
            state_a = estimator_a.estimate(geo_a)
            state_b = estimator_b.estimate(geo_b)

        assert state_a.initialized
        assert state_b.initialized

        lab_a = _lab_mean(state_a.appearance_latent)
        lab_b = _lab_mean(state_b.appearance_latent)
        distance = _lab_distance(lab_a, lab_b)

        assert distance > 15.0, f"Estimator outputs not distinguishable: distance={distance:.2f}"


# ─── Test Class 3: Renderer Blending Equation ───────────────────────────────

class TestRendererBlendingEquation:
    """Verify the blending equation Y = M * Y_face + (1-M) * Y_bg.

    These tests use known inputs to verify the renderer produces
    mathematically correct output.
    """

    def test_blend_full_mask_returns_identity(self):
        """When mask=1 everywhere, output should be identity face."""
        renderer = Renderer()

        # Create source frame (blue)
        source = np.full((360, 640, 3), (255, 0, 0), dtype=np.uint8)

        # Create identity face (red) in canonical space
        identity_face = np.full((256, 256, 3), (0, 0, 255), dtype=np.uint8)

        # Full mask (everywhere 1.0)
        mask = np.ones((256, 256), dtype=np.float32)

        geo = _make_geometry_state(identity_face, mask=mask)
        ...
        temporal = _make_temporal_state(confidence=1.0)
        crop = _make_crop_plan()

        output = renderer.render(source, geo, identity_state, temporal, crop)

        # Output should be closer to red than blue in the face region
        # (after crop and warp, the identity should dominate)
        assert output.shape[0] == 1920
        assert output.shape[1] == 1080

    def test_blend_zero_mask_returns_source(self):
        """When mask=0 everywhere, output should be source (enhanced)."""
        renderer = Renderer()

        # Create source frame with distinct color
        source = np.full((360, 640, 3), (100, 150, 200), dtype=np.uint8)

        # Create identity face
        identity_face = np.full((256, 256, 3), (0, 0, 255), dtype=np.uint8)

        # Zero mask at canonical size
        mask = np.zeros((256, 256), dtype=np.float32)

        # Use empty semantic_regions to avoid size mismatch
        geo = GeometryState(
            landmarks_478=None,
            landmarks=None,
            pose=(0.0, 0.0, 0.0),
            canonical_transform=np.eye(3, dtype=np.float32),
            inverse_transform=np.array([[1, 0, 0], [0, 1, 0]], dtype=np.float32),
            crop_transform=_make_crop_plan(),
            mesh=None,
            semantic_regions=None,
            mask=mask,
            geometry_confidence=0.9,
            canonical_face=identity_face,
        )
        identity_state = IdentityState(
            anchor_basis=[identity_face],
            anchor_weights=[1.0],
        ...
        # Output should be the source (cropped and enhanced, but not identity-blended)
        assert output.shape[0] == 1920
        assert output.shape[1] == 1080

    def test_blend_partial_mask_is_weighted(self):
        """With mask=0.5, output should be roughly average of source and identity."""
        renderer = Renderer()

        # Source: uniform blue
        source = np.full((360, 640, 3), (200, 0, 0), dtype=np.uint8)

        # Identity: uniform green
        identity_face = np.full((256, 256, 3), (0, 200, 0), dtype=np.uint8)

        # Partial mask at canonical size
        mask = np.full((256, 256), 0.5, dtype=np.float32)
    ...
    def test_renderer_preserves_output_contract(self):
        """Renderer must always produce (1920, 1080, 3) uint8."""
        renderer = Renderer()
        ...
        for conf in [0.0, 0.3, 0.5, 0.7, 1.0]:
            ...
            identity_state = IdentityState(
                anchor_basis=[face],
                anchor_weights=[1.0],
                appearance_latent=face,
                region_confidence={},
                identity_uncertainty=1.0 - conf,
                initialized=True,
            )
            temporal = _make_temporal_state(confidence=conf)
            crop = _make_crop_plan()

            output = renderer.render(source, geo, identity_state, temporal, crop)

            assert output.shape == (1920, 1080, 3), f"Shape violation at conf={conf}: {output.shape}"
            assert output.dtype == np.uint8, f"Dtype violation at conf={conf}: {output.dtype}"
            assert not np.any(np.isnan(output)), f"NaN at conf={conf}"
            assert not np.any(np.isinf(output)), f"Inf at conf={conf}"

    def test_temporal_confidence_modulates_blend(self):
        """Lower temporal confidence should reduce identity contribution."""
        renderer = Renderer()

        source = np.full((360, 640, 3), (100, 100, 100), dtype=np.uint8)
        identity_face = np.full((256, 256, 3), (200, 200, 200), dtype=np.uint8)
        mask = np.ones((256, 256), dtype=np.float32)

        # High confidence
        geo_high = _make_geometry_state(identity_face, mask=mask)
        temporal_high = _make_temporal_state(confidence=1.0)
        identity_state = IdentityState(
            anchor_basis=[identity_face],
            anchor_weights=[1.0],
            appearance_latent=identity_face,
        ...
        crop = _make_crop_plan()
        output_high = renderer.render(source, geo_high, identity_state, temporal_high, crop)

        # Low confidence
        temporal_low = _make_temporal_state(confidence=0.3)
        output_low = renderer.render(source, geo_high, identity_state, temporal_low, crop)

        # Both should satisfy contract
        assert output_high.shape == (1920, 1080, 3)
        assert output_low.shape == (1920, 1080, 3)
...
# ─── Test Class 5: Renderer With Identity Data ──────────────────────────────

class TestRendererWithIdentity:
    """Test renderer identity path with actual identity state data."""

    def test_identity_path_exercised(self):
        """Renderer should use identity path when initialized=True."""
        renderer = Renderer()

        source = np.full((360, 640, 3), (100, 100, 100), dtype=np.uint8)
        identity_face = np.full((256, 256, 3), (200, 180, 160), dtype=np.uint8)
        mask = np.ones((256, 256), dtype=np.float32) * 0.8

        # Use properly sized semantic regions
        geo = _make_geometry_state(identity_face, mask=mask)
        identity_state = IdentityState(
            anchor_basis=[identity_face],
            anchor_weights=[1.0],
            appearance_latent=identity_face,
            region_confidence={"skin": 0.9, "left_eye": 0.8},
            identity_uncertainty=0.1,
            initialized=True,
        )
        temporal = _make_temporal_state(confidence=0.9)
        crop = _make_crop_plan()
    ...
    def test_identity_path_with_low_confidence(self):
        """Renderer should handle low temporal confidence gracefully."""
        renderer = Renderer()
        ...
        identity_state = IdentityState(
            ...
            appearance_latent=identity_face,
            region_confidence={},
            identity_uncertainty=0.8,
            initialized=True,
        )
        # Very low confidence
        temporal = _make_temporal_state(confidence=0.1)
        crop = _make_crop_plan()
    ...
    def test_identity_path_with_no_mask(self):
        """Renderer should handle missing mask gracefully."""
        renderer = Renderer()

        source = np.full((360, 640, 3), (100, 100, 100), dtype=np.uint8)
        identity_face = np.full((256, 256, 3), (200, 180, 160), dtype=np.uint8)

        # Geometry state with no mask and no semantic regions
        geo = GeometryState(
            landmarks_478=None,
            landmarks=None,
            pose=(0.0, 0.0, 0.0),
    ...
    def test_enhancement_only_path(self):
        """Renderer should use enhancement path when identity not initialized."""
        renderer = Renderer()
        ...
        geo = GeometryState(
            ...
            mask=None,
            geometry_confidence=0.9,
            canonical_face=np.zeros((256, 256, 3), dtype=np.uint8),
        )
        identity_state = IdentityState(
            anchor_basis=[],
            anchor_weights=[],
...
# ─── Test Class 6: Temporal State Properties ─────────────────────────────────

class TestTemporalStateProperties:
    """Test temporal state mathematical properties."""

    def test_temporal_confidence_bounded(self):
        """Temporal confidence must be in [0, 1]."""
        estimator = TemporalEstimator()

        geo = _make_geometry_state(np.zeros((256, 256, 3), dtype=np.uint8))
        identity = IdentityState(
            anchor_basis=[],
            anchor_weights=[],
            appearance_latent=None,
            region_confidence={},
            identity_uncertainty=0.5,
            initialized=False,
        )

        for _ in range(100):
            state = estimator.estimate(geo, identity, None)
            assert 0.0 <= state.temporal_confidence <= 1.0, \
                f"Confidence out of bounds: {state.temporal_confidence}"

    def test_drift_score_non_negative(self):
        """Drift score must be >= 0."""
        estimator = TemporalEstimator()
        ...
        state = estimator.estimate(geo, identity, None)
        assert state.drift_score >= 0.0, f"Negative drift: {state.drift_score}"

    def test_continuity_score_bounded(self):
        """Continuity score must be in [0, 1]."""
        estimator = TemporalEstimator()
        ...
        assert 0.0 <= state.continuity_score <= 1.0, \
            f"Continuity out of bounds: {state.continuity_score}"
