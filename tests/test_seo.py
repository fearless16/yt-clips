...
def test_root_seo_stub_is_clean_re_export():
    ...
    from automation.seo.seo import (
        SEOGenerationError, process_all_seo, generate_seo_for_exported_clip,
        _enforce_limits,
    )
    ...
    with pytest.raises(ImportError):
        import seo  # noqa: root-level stub removed
    # Should NOT have safe_defaults padding in _enforce_limits
    import inspect
    src = inspect.getsource(_enforce_limits)
    assert "safe_defaults" not in src, \
        "_enforce_limits must not pad with safe_defaults"
