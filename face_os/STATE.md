...
| D-05 | Identity decoupling | **DEFAULT FLIPPED TO LATENT — Phase 3 complete, Phase 4 cleanup done** | Default `render_source='latent'` (pipeline.py:252). BeliefPixel demoted (`USE_LEGACY_RGB_BELIEF=False`). 0.4 albedo blend + shading sanitizer retired on latent path. Color-cast compensation with reject-on-failure guard. Silent sanitizers removed from `_ensure_shading` + pipeline; `assert_intrinsic_contract` sole guard. ABComparator wired: `_run_pipeline_source` + `compare_render_sources` (SSIM, LAB drift, sharpness, flicker). Slow test `TestLatentQualityOnRealVideo`: 90.5% face frames `latent_primary=True`, mean `source_pixel_fraction=0.236`. Fast suite: 282 passed, 9 skipped, 14 slow deselected. See `tasks.md` for full evidence.
