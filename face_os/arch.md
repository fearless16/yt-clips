...
| 16.8 | Composite C_recon | PARTIAL | `compute_reconstruction_confidence` `reconstruction_confidence.py` = `C_obs·Coverage_pose·Coverage_light·mean_visibility`; all four factors now real and wired; surfaced as telemetry `c_recon` (`types.py`, wired `pipeline.py:_emit_frame_telemetry`). E2E test_clip 60f: 0.0018 (correct: 0.34×0.05×0.11×0.90). Invariant `c_recon ≤ C_obs` tested. Phase-2B gate not yet wired (signal-only) | Gate reads `c_recon` but default flip is Phase-2B decision (§19) |
