...
|---|---------|--------|----------------------|-----|
| 16.1 | Observation model O=R(I,p,l)+ε | PARTIAL→PRESENT | Forward-model residual `compute_observation_residual` `observation_model.py` = `‖O_t − Ô_t‖` in LAB space over face interior; per-pixel ε_t as LAB difference; `observation_confidence = exp(−residual/30)` surfaced as telemetry (`types.py` `LatentRenderTelemetry`, wired `pipeline.py:_render_with_latent`). Kalman R on 7-dim scalar features `state_evolution.py:64`, `pipeline.py:1717` still exists. Image `reconstruct()` `intrinsic_decomposition.py:164` still auto-encoder round-trip. | Forward model exists as a SIGNAL (residual + noise + confidence in telemetry) but NOT yet consumed by the update/gate (Phase-2C); ε is per-pixel scalar (not full covariance) |
